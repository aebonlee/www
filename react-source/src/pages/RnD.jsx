...
const RnD = () => {
  ...
  const projects = [
    ...
    },
    {
      period: '2007',
      duration: isEn ? '6 months' : '6개월',
      title: isEn ? 'RFID · HSDPA Navigation System for the Visually Impaired' : 'RFID · HSDPA 시각장애인 위치정보 내비게이션 시스템 개발',
      type: isEn ? 'Industry-Academia-Research Consortium' : '산·학·연 공동기술개발 컨소시엄',
      desc: isEn
        ? 'Developed a navigation system for visually impaired individuals using RFID (Radio-Frequency Identification) and HSDPA (High-Speed Downlink Packet Access) technologies. Combined indoor RFID positioning with outdoor HSDPA location services for seamless navigation.'
        : 'RFID(무선 주파수 인식) 기술과 HSDPA(고속하향패킷전송) 기술을 활용한 시각장애인용 내비게이션 시스템을 개발하였습니다. 실내 RFID 측위와 실외 HSDPA 위치 서비스를 결합하여 끊김 없는 내비게이션을 구현했습니다.',
      techs: ['RFID', 'HSDPA', 'GPS', 'Accessibility', 'IoT'],
  ...
  return (
    <>
      ...
      <section style={{ padding: '80px 0', background: 'var(--bg-white)' }}>
        <div className="container">
          <div className="section-header" data-aos="fade-up">
            <h2 className="section-title">{isEn ? 'R&D Project Portfolio' : '연구개발 과제 수행 내역'}</h2>
            <p className="section-subtitle">
              {isEn
                ? 'Government-funded Industry-Academia-Research consortium projects'
                : '정부지원 산·학·연 공동기술개발 컨소시엄 과제'}
            </p>
          </div>

          <div className="rnd-projects">
            {projects.map((project, i) => (
              <div key={i} className="rnd-project-card" data-aos="fade-up" data-aos-delay={i * 80}>
                <div className="rnd-project-header">
                  <div className="rnd-project-icon">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">{project.icon}</svg>
                  </div>
                  <div className="rnd-project-meta">
                    <span className="rnd-period">{project.period}</span>
                    <span className="rnd-duration">{project.duration}</span>
                  </div>
                </div>
                <h3 className="rnd-project-title">{project.title}</h3>
                <p className="rnd-project-type">{project.type}</p>
                <p className="rnd-project-desc">{project.desc}</p>
                <div className="rnd-tech-tags">
                  {project.techs.map((tech, j) => (
                    <span key={j} className="rnd-tech-tag">{tech}</span>
                  ))}
                </div>
              </div>
            ))}
      ...
      <CTA
        title={isEn ? 'Looking for R&D Partnership?' : '연구개발 협력이 필요하신가요?'}
        subtitle={isEn ? 'Leverage our proven R&D expertise for your next project' : '검증된 연구개발 역량으로 프로젝트를 함께합니다'}
      />
    </>
  );
};
