...
- [x] 애널리틱스 — GA4 SPA 페이지뷰 추적 (ID 적용 완료)
- [x] 검색엔진 — 네이버/구글 인증 완료 + sitemap 제출 완료

---

## 세션 39: SEO 후속 조치 완료 — GA4·네이버·구글 인증 + OG 이미지

### 작업 배경

세션 38에서 SEO 인프라를 구축했으나, GA4 Measurement ID, 네이버 인증 코드, OG 이미지 PNG, Google Search Console 인증이 placeholder 상태로 남아있었음. 본 세션에서 전부 실제 값으로 교체 및 등록 완료.

### 작업 내역

#### 1. GA4 Measurement ID 적용

| 항목 | 변경 |
|------|------|
| 교체 대상 | `G-XXXXXXXXXX` → `G-EQR45N4R70` |
| 수정 파일 | `react-source/index.html`, `D:/www/index.html` (각 2곳) |
| 주석 정리 | `<!-- GA4 (발급 후 G-XXXXXXXXXX 교체) -->` → `<!-- GA4 -->` |

**커밋**: `b44ddaf` chore: GA4 Measurement ID 적용 (G-EQR45N4R70)

#### 2. 네이버 Search Advisor 사이트 인증

| 항목 | 변경 |
|------|------|
| 교체 대상 | `VERIFICATION_CODE` → `c950825244b02372e30b3083dcca63c261b87eb2` |
| 수정 파일 | `react-source/index.html`, `D:/www/index.html` |
| 주석 정리 | `<!-- 네이버 사이트 인증 (발급 후 VERIFICATION_CODE 교체) -->` → `<!-- 네이버 사이트 인증 -->` |
| 후속 조치 | searchadvisor.naver.com에서 소유확인 완료 + `sitemap.xml` 제출 완료 |

**커밋**: `605aad1` chore: 네이버 Search Advisor 사이트 인증 코드 적용

#### 3. OG 기본 이미지 PNG 추가

| 항목 | 내용 |
|------|------|
| 파일 | `og-default.png` (1200×630px) |
| 저장 경로 | `react-source/public/assets/images/og-default.png`, `D:/www/assets/images/og-default.png` |
| 용도 | 소셜 미디어 공유 시 표시되는 기본 이미지 (카카오톡, 페이스북, 트위터 등) |

**커밋**: `1ade119` chore: OG 기본 이미지 PNG 추가 (1200x630)

#### 4. Google Search Console 인증 파일 추가

| 항목 | 내용 |
|------|------|
| 파일 | `googlee2a105e505ac51ad.html` |
| 내용 | `google-site-verification: googlee2a105e505ac51ad.html` |
| 저장 경로 | `D:/www/googlee2a105e505ac51ad.html` |
| 후속 조치 | Google Search Console에서 소유확인 + sitemap.xml 제출 예정 |

#### 5. 최신 빌드 배포

- `npx vite build` 성공 (158 모듈, 3.63초)
- `dist/*` → `D:/www/` 전체 배포 (assets, index.html, 404.html, robots.txt, sitemap.xml, CNAME, favicon.svg)

**커밋**: `7b74b56` deploy: 최신 빌드 배포 + Google Search Console 인증 파일 추가
...
|------|------|
| 홈페이지 접속 (200) | 정상 |
| GA4 `G-EQR45N4R70` | 정상 — 실제 ID 확인 |
| JSON-LD 구조화 데이터 | 정상 — Organization + WebSite |
| SPA 디코드 스크립트 | 정상 |
| robots.txt | 정상 — 7개 비공개 경로 차단 + sitemap 링크 |
| sitemap.xml | 정상 — 24개 URL |
| Google 인증 파일 | 정상 — `googlee2a105e505ac51ad.html` 접근 가능 |
| OG/Twitter/canonical 메타 태그 | 정상 — index.html에 포함 확인 |

### 세션 39 커밋 이력

| 순서 | 커밋 해시 | 내용 |
|------|----------|------|
| 1 | `b44ddaf` | chore: GA4 Measurement ID 적용 (G-EQR45N4R70) |
| 2 | `c1ffeaf` | deploy: GA4 Measurement ID 적용 |
| 3 | `605aad1` | chore: 네이버 Search Advisor 사이트 인증 코드 적용 |
| 4 | `8557903` | deploy: 네이버 Search Advisor 사이트 인증 코드 적용 |
| 5 | `1ade119` | chore: OG 기본 이미지 PNG 추가 (1200x630) |
| 6 | `3e82895` | deploy: OG 기본 이미지 PNG 추가 |
| 7 | `7b74b56` | deploy: 최신 빌드 배포 + Google Search Console 인증 파일 추가 |

### SEO 인프라 최종 상태

| 항목 | 상태 |
|------|------|
| HashRouter → BrowserRouter | 완료 |
| react-helmet-async + SEOHead (20페이지) | 완료 |
| OG + Twitter Card 메타 태그 | 완료 |
| OG 기본 이미지 PNG (1200×630) | 완료 |
| canonical URL | 완료 |
| robots.txt + sitemap.xml | 완료 |
| GA4 `G-EQR45N4R70` | 적용 완료 |
| 네이버 Search Advisor 인증 + sitemap 제출 | 완료 |
| Google Search Console 인증 파일 | 배포 완료 |
| JSON-LD (Organization + WebSite) | 완료 |
| usePageTracking SPA 페이지뷰 추적 | 완료 |

---

## 세션 40: 관리자 권한 이메일 기반 전환 + CRUD 버튼 목록 노출

### 작업 배경

갤러리·블로그·게시판에서 CRUD 기능(수정/삭제)을 목록 화면 아래쪽에 바로 보이도록 개선 요청. 동시에 관리자 권한을 DB role 기반에서 **특정 이메일 2개**로만 한정하는 요구.

### 1. 관리자 권한 변경 (`AuthContext.jsx`)

| Before | After |
|--------|-------|
| `profile?.role === 'admin'` (DB user_profiles.role 기반) | 이메일 기반 화이트리스트 |
...
- `aebon@kakao.com` — 카카오 로그인 시 관리자
- `aebon@kyonggi.ac.kr` — 구글 로그인 시 관리자
- 그 외 모든 계정은 일반 사용자

### 2. 블로그 목록 CRUD 버튼 추가 (`Blog.jsx`)

| 변경 | 상세 |
|------|------|
| import 추가 | `useNavigate`, `deleteBlogPost` |
| 핸들러 추가 | `handleEdit(e, postId)`, `handleDelete(e, postId)` — stopPropagation + confirm |
| Featured 카드 | 하단에 수정/삭제 버튼 추가 (관리자만) |
| 일반 카드 | 하단에 수정/삭제 버튼 추가 (관리자만) |
| 삭제 후 | 목록 즉시 갱신 (`setPosts` 필터링) |

### 3. 게시판 목록 CRUD 컬럼 추가 (`Board.jsx`)

| 변경 | 상세 |
|------|------|
| import 추가 | `useNavigate`, `deleteBoardPost` |
| useAuth 변경 | `{ isLoggedIn }` → `{ isLoggedIn, isAdmin }` |
| 핸들러 추가 | `handleEdit(e, postId)`, `handleDeletePost(e, postId)` |
| 테이블 헤더 | "관리" 컬럼 추가 (관리자만 표시, width 100px) |
| 테이블 바디 | 각 행에 수정/삭제 버튼 추가 (관리자만 표시) |
| 삭제 후 | 목록 즉시 갱신 |

### 4. 갤러리 (`Gallery.jsx`)

변경 없음 — 이미 카드 하단에 수정/삭제 아이콘 버튼이 구현되어 있었음.

### 수정 파일

| 파일 | 변경 |
|------|------|
| `src/contexts/AuthContext.jsx` | isAdmin 로직: role 기반 → 이메일 화이트리스트 |
| `src/pages/Blog.jsx` | 목록 카드 하단 수정/삭제 버튼 + deleteBlogPost import |
| `src/pages/Board.jsx` | 테이블 관리 컬럼 + 행별 수정/삭제 버튼 + deleteBoardPost import |

### 커밋

| 순서 | 커밋 해시 | 내용 |
|------|----------|------|
| 1 | `b93067c` | feat: 관리자 권한을 이메일 기반으로 변경 + 블로그/게시판 목록에 수정/삭제 버튼 추가 |
| 2 | `9c33763` | deploy: 관리자 권한 이메일 기반 변경 + CRUD 버튼 추가 빌드 배포 |
